...
deploy:
	...
	@FNOX_AGE_KEY=$$(cat ~/.config/fnox/age.txt | grep "AGE-SECRET-KEY") ssh ale@achih1 "cd /home/ale/landing-new-v2 && git pull && export FNOX_AGE_KEY='$$FNOX_AGE_KEY' && docker compose build && docker compose down"
	@echo "Starting containers with secrets (FNOX_AGE_KEY passed directly)..."
	@FNOX_AGE_KEY=$$(cat ~/.config/fnox/age.txt | grep "AGE-SECRET-KEY") ssh ale@achih1 "cd /home/ale/landing-new-v2 && export FNOX_AGE_KEY='$$FNOX_AGE_KEY' && docker compose up -d"
